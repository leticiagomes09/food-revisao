...
export const create = async (req, res) => {
    try {
        ...
        if (!descricao) return res.status(400).json({ error: 'A descricao (descricao) é obrigatório!' });
        if (!categoria) return res.status(400).json({ error: 'A categoria (categoria) é obrigatório!' });
